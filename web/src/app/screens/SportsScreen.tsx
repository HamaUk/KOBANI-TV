...
export function SportsScreen() {
  ...
  const visible = useMemo(() => {
    if (!channels) return [];
    let list = channels.filter(isSportChannel);
    ...
    return list;
  }, [channels, debouncedSearch, epgMap]);

  useNumericRemote((num) => {
    const target = visible.find((c) => c.number === num);
  ...
  if (visible.length === 0) return <div className="empty">No sports channels found.</div>;

  return (
    <div>
      <div style={{ display: "flex", gap: 8, marginBottom: 12, alignItems: "center" }}>
        <h2 style={{ margin: 0 }}>{t("nav.sports")}</h2>
